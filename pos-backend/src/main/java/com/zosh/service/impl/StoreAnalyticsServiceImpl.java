...
//        return null;
    ...
    @Override
    public List<RecentBranchSalesDTO> getRecentSales(Long storeAdminId) {
        LocalDate today = LocalDate.now();
        return orderRepository.findRecentBranchSales(storeAdminId).stream()
                .limit(4)
                .map(item -> RecentBranchSalesDTO.builder()
                        .branchId(item.getBranchId())
                        .branchName(item.getBranchName())
                        .totalSales(item.getTotalSales())
                        .lastSaleAt(item.getLastSaleAt())
                        .periodLabel(labelFor(item.getLastSaleAt(), today))
                        .build())
                .collect(Collectors.toList());
    }
...
//                .topBranch(branchRepository.findTopBranchBySales(storeAdminId))
    ...
    private String labelFor(LocalDateTime dateTime, LocalDate today) {
        if (dateTime == null) {
            return "No Sales";
        }

        long days = ChronoUnit.DAYS.between(dateTime.toLocalDate(), today);
        if (days == 0) {
            return "Today";
        }
        if (days == 1) {
