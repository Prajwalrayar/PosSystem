...
import { updateProfile as updateProfileApi, changePassword as changePasswordApi } from '@/utils/api';
...
export const updateProfile = createAsyncThunk(
  ...
  async ({ fullName, phone }, { rejectWithValue }) => {
    try {
      const payload = await updateProfileApi({ fullName, phone });
      return payload.data;
    } catch (error) {
...
export const changePassword = createAsyncThunk(
  ...
  async ({ currentPassword, newPassword, confirmPassword }, { rejectWithValue }) => {
    try {
      const payload = await changePasswordApi({ currentPassword, newPassword, confirmPassword });
      return payload.data;
    } catch (error) {
      return rejectWithValue(error.response?.data);
    }
