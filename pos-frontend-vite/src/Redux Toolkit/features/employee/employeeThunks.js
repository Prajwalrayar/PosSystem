...
import { resetEmployeePassword as resetEmployeePasswordApi } from "@/utils/api";
import { getEmployeePerformance as getEmployeePerformanceApi } from "@/utils/api";
...
export const resetEmployeePassword = createAsyncThunk(
  ...
  async ({ employeeId, temporaryPassword, forceChangeOnNextLogin }, { rejectWithValue }) => {
    try {
      const payload = await resetEmployeePasswordApi(employeeId, {
        temporaryPassword,
...
export const fetchEmployeePerformance = createAsyncThunk(
  'employee/fetchPerformance',
  async ({ employeeId, from, to }, { rejectWithValue }) => {
    try {
      const res = await getEmployeePerformanceApi(employeeId, { from, to });
      return res.data;
    } catch (err) {
