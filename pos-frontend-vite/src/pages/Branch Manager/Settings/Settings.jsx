...
const Settings = () => {
  ...
  const getPayloadByType = (settingType) => {
    switch (settingType) {
      ...
      case "payment":
        return { payment: paymentSettings };
      case "discount":
        return { discount: discountSettings };
      default:
        return {
  ...
  const validatePayload = (payload) => {
    ...
    if (payload.tax?.gstEnabled && !payload.tax?.gstPercentage) {
      errors["tax.gstPercentage"] = "GST percentage is required when GST is enabled";
    }
    if (payload.payment?.acceptUPI && !payload.payment?.upiId?.trim()) {
      errors["payment.upiId"] = "UPI ID is required when UPI is enabled";
    }
    if (payload.payment?.acceptCard && !payload.payment?.cardTerminalId?.trim()) {
      errors["payment.cardTerminalId"] = "Card terminal ID is required when card is enabled";
    }
    if (payload.discount?.allowDiscount && !payload.discount?.maxDiscountPercentage) {
      errors["discount.maxDiscountPercentage"] = "Max discount percentage is required when discount is enabled";
    }
  ...
  const handleSaveSettings = async (settingType) => {
    ...
    if (!branchId) {
      toast({
        ...
        description: "Branch information is unavailable.",
        variant: "destructive",
      });
      return;
    }
    ...
    setFieldErrors({});
    dispatch(clearBranchSettingsState());

    const payload = getPayloadByType(settingType);
    ...
    try {
      ...
      if (data?.tax) {
        setTaxSettings((prev) => ({ ...prev, ...data.tax }));
      }
      if (data?.payment) {
        setPaymentSettings((prev) => ({ ...prev, ...data.payment }));
      }
      if (data?.discount) {
        setDiscountSettings((prev) => ({ ...prev, ...data.discount }));
      }
            ...
            <CardContent className="space-y-6">
              ...
              <div className="flex justify-end">
                <Button
                  ...
                  disabled={branchSettings.saving}
                >
                  <Save className="h-4 w-4" />
                  {branchSettings.saving ? "Saving..." : "Save Changes"}
                </Button>
              </div>
            </CardContent>
          </Card>
                ...
                {taxSettings.gstEnabled && (
                  <div className="space-y-4 pl-6 border-l-2 border-gray-100">
                    <div className="space-y-2">
                      ...
                      />
                      {fieldErrors["tax.gstPercentage"] && (
                        <p className="text-xs text-red-600">{fieldErrors["tax.gstPercentage"]}</p>
                      )}
                    </div>
              ...
              <div className="space-y-4">
                ...
                {paymentSettings.acceptUPI && (
                  <div className="space-y-2 pl-6 border-l-2 border-gray-100">
                    ...
                    />
                    {fieldErrors["payment.upiId"] && (
                      <p className="text-xs text-red-600">{fieldErrors["payment.upiId"]}</p>
                    )}
                  </div>
                )}
                ...
                {discountSettings.allowDiscount && (
                  <div className="space-y-4 pl-6 border-l-2 border-gray-100">
                    <div className="space-y-2">
                      ...
                      />
                      {fieldErrors["discount.maxDiscountPercentage"] && (
                        <p className="text-xs text-red-600">{fieldErrors["discount.maxDiscountPercentage"]}</p>
                      )}
                    </div>
