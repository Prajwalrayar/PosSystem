...
import { getProductById } from "@/Redux Toolkit/features/product/productThunks";
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export default function Products() {
  ...
  const openEditDialog = async (product) => {
    try {
      const fullProduct = await dispatch(getProductById(product.id)).unwrap();
      setCurrentProduct(fullProduct);
      setIsEditDialogOpen(true);
    } catch (err) {
      toast({
        title: "Error",
        description: err || "Failed to load product details",
        variant: "destructive",
      });
    }
  };
