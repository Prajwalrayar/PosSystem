...
const SyncValues = ({ values, onValuesChange }) => {
  const lastValuesRef = useRef("");

  useEffect(() => {
    const serialized = JSON.stringify(values);
    if (onValuesChange && serialized !== lastValuesRef.current) {
...
const StoreSettingsForm = ({ initialValues, onValuesChange }) => {

  const formKey = initialValues?.storeName || "store-settings-form";
    ...
    >
      {({ errors, touched, values }) => (
        <Form className="space-y-6">
          <SyncValues values={values} onValuesChange={onValuesChange} />
