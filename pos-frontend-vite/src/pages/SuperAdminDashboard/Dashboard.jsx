  ...
  const recentActivities = [
    ...(storeRegistrationStats || [])
      ...
      .map((item) => ({
        ...
        message: `Store registrations: ${item.count || item.value || 0}`,
        time: formatRelativeTime(item.date || item.day || item.label),
      })),
    ...(dashboardSummary?.pendingStores > 0
      ? [
          {
            colorClass: "bg-yellow-500",
            message: `${dashboardSummary.pendingStores} pending store request${dashboardSummary.pendingStores > 1 ? "s" : ""}`,
            time: "Current",
          },
        ]
      : []),
    ...(dashboardSummary?.blockedStores > 0
      ? [
          {
            colorClass: "bg-red-500",
            message: `${dashboardSummary.blockedStores} blocked store${dashboardSummary.blockedStores > 1 ? "s" : ""}`,
            time: "Current",
          },
        ]
      : []),
          ...
          {recentActivities.length === 0 ? (
            ...
            <div className="space-y-4">
              {recentActivities.map((activity, index) => (
                <div key={`${activity.message}-${index}`} className="flex items-center gap-4 p-4 border rounded-lg">
