...
const validationSchema = Yup.object().shape({
  ...
  extraFeatures: Yup.array().of(Yup.string().trim()),
});
...
const EditPlanDialog = ({ open, onOpenChange, plan, onSuccess }) => {
  ...
  const sanitizeExtraFeatures = (extraFeatures = []) =>
    extraFeatures.map((item) => item.trim()).filter(Boolean);
  ...
  const handleSubmit = async (values, { setSubmitting, setErrors }) => {
    ...
    try {
      const payload = {
        ...values,
        extraFeatures: sanitizeExtraFeatures(values.extraFeatures),
      };
      const res = await dispatch(updateSubscriptionPlan({ id: plan.id, plan: payload }));
