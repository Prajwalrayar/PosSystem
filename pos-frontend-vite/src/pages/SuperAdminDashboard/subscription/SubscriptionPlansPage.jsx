...
const SubscriptionPlansPage = () => {
  ...
  const handleStatusToggle = async (plan) => {
    ...
    const currentActive = statusOverrides[plan.id] ?? plan.active ?? false;
    const nextActive = !currentActive;
    const updated = { ...plan, active: nextActive };
          ...
          <TableBody>
            ...
            {filteredPlans.map((plan) => (
              <TableRow key={plan.id} className="border-b hover:bg-gray-50">
                {(() => {
                  const resolvedActive = statusOverrides[plan.id] ?? plan.active ?? false;
                  return null;
                })()}
                  ...
                  <div className="flex items-center gap-2">
                    {(() => {
                      ...
                      return (
                        <>
                    <Switch
                      checked={!!resolvedActive}
                      onCheckedChange={() => handleStatusToggle(plan)}
                      disabled={statusLoadingId === plan.id}
                    />
                    <span className={resolvedActive ? 'text-green-600' : 'text-red-500'}>
